...
- **Cheat-sheet card** — the shortcuts where the user already is, instead of behind a toolbar-icon click. Mounts top-centre of the player frame once it has mounted (same probe as the feedback pill), collapsed: icon, title and one key to try (`Try V to mute Vocals`). "Show keys" expands it to the full table — the same rows as the popup, from the shared `collectTrackKeys` so the two cannot drift — plus the player's own keys (`PLAYER_NATIVE_SHORTCUTS`: Space = play/pause), which the extension lists but never intercepts. Its footer carries the two ways to reach us — a star row linking to the Web Store review page and the feedback form — so the extension keeps **one** floating surface instead of three and never covers the player's own controls. Once the shortcuts have proved useful (`REVIEW_PROMPT_AFTER` successful toggles) the collapsed line swaps the teaser for the rating ask in the space it already occupied; `refreshCheatSheet` flips it the moment the threshold is crossed. The stars are a **link, not an input**: all five render identically and none fills on hover or click, because the Web Store takes the rating on its own page and claiming to have captured one would be a lie. The ✕ **minimises rather than dismisses**: the card shrinks to a round launcher carrying the same icon, in the card's own position, and a click brings it back (`moises-kb-cheatsheet-minimized`). Shrinking in place is what makes the launcher legible — parked in a corner it reads as unrelated page chrome. There is no way to remove it entirely, deliberately: the card is the only home of the shortcuts, the feedback form and the rating, so a dead end there would strand all three. Closing **while the rating is being asked** counts as a "no thanks" and is honoured for good; the card returns teaching. Using the shortcuts does not retire it. Sits below the player toolbar, not over it, and fades in after a forced reflow rather than a `requestAnimationFrame`, which never runs while the tab is in the background.
